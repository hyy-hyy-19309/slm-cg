...
class SLM(object):
    ...
    def __init__(self, NT, initial_phi=None, profile_s=None):
        ...
        if initial_phi is None:
            initial_phi = np.random.uniform(low=0, high=2 * np.pi,
                                            size=(self.n_pixels ** 2))  # input phase set to random if none given

        assert profile_s.shape == (
            self.n_pixels, self.n_pixels), 'profile_s is wrong shape, should be ({n},{n})'.format(n=self.n_pixels)
        self.profile_s_r = profile_s.real.astype('float64')
        self.profile_s_i = profile_s.imag.astype('float64')

        assert initial_phi.shape == (
            self.n_pixels ** 2,), "initial_phi must be a vector of phases of size N^2 (not (N,N)).  Shape is " + str(
            initial_phi.shape)
...
def laser_gaussian(n, r0, sigmax, sigmay, A=1.0, save_param=False):
    ...
    if save_param:
        param_used = "target_gaussian | n={0} | r0={1} | sigmax={2} | sigmay={3} | A={4} ".format(n, r0, sigmax, sigmay,
                                                                                                  A)
        return z, param_used
    else:
        return z
...
def target_lg(n, r0List, w, l, A, save_param=False):
    """
    Create n x n target:
...
def target_ringlattice(n, r0, sigma, d, nb_spots=12., A=1.0, save_param=False):
    ...
    if save_param:
        param_used = "target_ringlattice | n={0} | r0={1} | sigma={2} | d={3} | nb_spots={4} | A={5}".format(n, r0,
                                                                                                             sigma, d,
                                                                                                             nb_spots,
                                                                                                             A)
        return z, param_used
...
def target_squarelattice(n, r0, sigma, d, dim=6, A=1.0, save_param=False):
    ...
    for n0 in range(0, dim):
        for n1 in range(0, dim):
            spot = np.exp(
                - (np.power((((X - r0[0]) + x1[n0]) / sigma), 2) + np.power((((Y - r0[1]) + y1[n1]) / sigma), 2)))
            z = z + spot

    if save_param:
        param_used = "target_squarelattice | n={0} | r0={1} | sigma={2} | d={3} | dim={4} | A={5}".format(n, r0, sigma,
                                                                                                          d, dim, A)
        return z, param_used
    else:
        return z
...
def gaussian_top_square(n, r0, dx, dy, sigmax, sigmay, A=1.0, save_param=False):
    ...
    if save_param:
        param_used = "gaussian_line | n={0} | r0={1} | dx={2} | dy={3} | sigmax={4} | sigmay={5}| A={6}".format(n, r0,
                                                                                                                dx, dy,
                                                                                                                sigmax,
                                                                                                                sigmay,
                                                                                                                A)
...
def gaussian_top_round(n, r0List, d, sigma, A=1.0, save_param=False):
    """
    Create n x n target: 
...
def graphene(n, r0, l=35, A=1., save_param=False):
    ...
    for i in range(0, 6):
        z = z + A * (-1) ** (i) * np.exp(
            1j * 2 * np.pi / l * ((X - r0[0]) * np.cos(2 * np.pi * i / 6) + (Y - r0[1]) * np.sin(2 * np.pi * i / 6)))
...
def hexagon(n, r0, d=35, A=1., save_param=False):
    ...
    # target definition
    z = (np.abs(X - r0[0]) <= d) & (np.abs((np.power(3, 0.5) * 0.5) * (Y - r0[1]) + (0.5) * (X - r0[0])) <= d) & (
            np.abs((np.power(3, 0.5) * 0.5) * (Y - r0[1]) - (0.5) * (X - r0[0])) <= d);
...
def n_plot(p, d=[], sc=[], v=[], t=[], c=[], save=False):
    ...
    for i in range(0, nbplot):
                ...
                if no_scale == True:
                    ...
                    if len(sc1D[j]) == 0:
                        ...
                        if len(sc1D[j]) != len(p1D[j]):
                            print
                            '        /\  \n       /  \ \nd=1 | /____\  sc[{0}][{1}] and p[{0}][{1}]have different sizes'.format(
                                i, j);
                            return
                        else:
                            ax.plot(sc1D[j], p1D[j], color=c1D[j])
        ...
        if d[i] == 3:
            ...
            if len(sc) == 0:
                print
                '        /\  \n       /  \ \nd=3 | /____\  we need to give plotting limits in arguments';
                return
            else:
                if len(sc[i]) == 0:
                    print
                    '        /\  \n       /  \ \nd=3 | /____\  we need to give plotting limits in sc[{0}]'.format(i);
                    return
                else:
                    p3D = p[i]
                    ...
                    # plot
                    cax = ax.plot_surface(X, Y, p3D[imin:imax, jmin:jmax], cmap=plt.get_cmap('jet'), rstride=1,
...
def Fidelity(weighting, target, phase, E_out_amp, E_out_p):
    ...
    F = np.sum(E_test_w * np.conjugate(E_out_w)) / np.power(
        (np.sum(np.power(np.abs(E_test_w), 2))) * (np.sum(np.power(np.abs(E_out_w), 2))), 0.5)
